...
description: >-
  Production-ready CSS transitions for web apps. Use when implementing or animating notification badges, dropdowns, modals, panel reveals, page transitions, card resizes, number pop-ins, text swaps, icon swaps, success checks, avatar group hovers, error/shake states, input clear, skeleton loaders, shimmer text, sliding tabs, tooltips, staggered text reveals, card hover tilt, plus-to-menu morph, or accordions. Triggers on any request to add, smooth, or fix UI motion — "add a transition", "animate the dropdown", "make the modal open smoothly", "make it animate", "fade between", "success animation", "shake on invalid", "hover lift", "loading shimmer", "segmented control", "expand / collapse", "disclosure" — and on motion-token work: "motion tokens", "tokenize my animations", "replace hardcoded durations". Also the commands transitions reveal, transitions review, transitions apply, transitions refine. Use for any component or prototype build involving UI motion.
---
