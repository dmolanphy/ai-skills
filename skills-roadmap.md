...
- `market-landscape-builder` — evidence-backed market/category landscape briefings: why a market matters now, what's broken about how it's served, opportunity size, and where the underserved gap sits. Triangulates across 5 evidence lenses (attitudinal, behavioral, market sizing, economic, competitive artifact) rather than leaning on any one kind of proof. Complements `buyer-journey-builder` (that one maps how the audience decides; this one establishes whether the fight's worth having).
- `seo-geo-audit-builder` — scored, verifiable technical SEO/GEO readiness audit against named competitors, with a live cross-engine citation test (Live Insight Pull) as its standout deliverable piece. Real fetches only — never a simulated citation score. Backported 2026-08-29 with 4 checks folded in from `seo-geo` (llms.txt presence, image alt text, URL structure, internal linking structure) plus a source-recency discipline; llms.txt and internal linking were new to this skill, alt text and URL structure were previously covered only by `seo-geo`'s broader technical checklist. The third leg alongside `market-landscape-builder` and `buyer-journey-builder` — a sellable diagnostic in its own right, deliberately kept separate from `seo-geo` (Develop), which is build-phase requirements-and-fixes work, not a strategy deliverable.
